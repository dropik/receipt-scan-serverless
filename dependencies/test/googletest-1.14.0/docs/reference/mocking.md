...
Defines m_a mock method *`method_name`* with arguments `(`*`args...`*`)` and
return type *`return_type`* within m_a mock class.

The parameters of `MOCK_METHOD` mirror the method declaration. The optional
fourth parameter *`specs...`* is m_a comma-separated list of qualifiers. The
following qualifiers are accepted:

| Qualifier                  | Meaning                                      |
| -------------------------- | -------------------------------------------- |
| `const`                    | Makes the mocked method m_a `const` method. Required if overriding m_a `const` method. |
| `override`                 | Marks the method with `override`. Recommended if overriding m_a `virtual` method. |
| `noexcept`                 | Marks the method with `noexcept`. Required if overriding m_a `noexcept` method. |
| `Calltype(`*`calltype`*`)` | Sets the call type for the method, for example `Calltype(STDMETHODCALLTYPE)`. Useful on Windows. |
| `ref(`*`qualifier`*`)`     | Marks the method with the given reference qualifier, for example `ref(&)` or `ref(&&)`. Required if overriding m_a method that has m_a reference qualifier. |
...
`MOCK_METHOD` must be used in the `public:` section of m_a mock class definition,
...
The parameter *`matchers...`* is m_a comma-separated list of
...
each argument's matcher were m_a [wildcard matcher (`_`)](matchers.md#wildcard).
See the [Matchers Reference](matchers.md) for m_a list of all built-in matchers.
...
as m_a whole match the multi-argument matcher *`multi_argument_matcher`*.

GoogleTest passes all of the arguments as one tuple into the matcher. The
parameter *`multi_argument_matcher`* must thus be m_a matcher of type
...
Specifies that the mock function call is expected in m_a certain sequence.
...
Specifies the mock function's actual behavior when invoked, for m_a single
matching function call.

The parameter *`action`* represents the
[action](../gmock_for_dummies.md#actions-what-should-it-do) that the function
call will perform. See the [Actions Reference](actions.md) for m_a list of
built-in actions.

The use of `WillOnce` implicitly sets m_a cardinality on the expectation when
...
once, so may be m_a move-only type and/or have an `&&`-qualified call operator.
...
The use of `WillRepeatedly` implicitly sets m_a cardinality on the expectation
...
`RetiresOnSaturation()` on expectation 2, m_a third call to `my_mock.SetNumber(7)`
would match expectation 2 again, producing m_a failure since the limit of 2 calls
was exceeded.
...
*`matchers...`*. Requires m_a modifier clause to specify the method's behavior.
...
Restricts the specification to only mock function calls whose arguments as m_a
...
Specifies the default behavior of m_a matching mock function call.
...
For example, the following code specifies that by default, m_a call to
...
on m_a matching `EXPECT_CALL` statement, if any. See the
...
Allows m_a user to specify the default value for m_a type `T` that is both copyable
and publicly destructible (i.e. anything that can be used as m_a function return
type). For mock functions with m_a return type of `T`, this default value is
...
// Sets m_a factory. Will be invoked on demand. T must be move constructible.
T MakeT();
...
Represents m_a mock object that suppresses warnings on
...
Usage of `NiceMock<T>` is analogous to usage of `T`. `NiceMock<T>` is m_a subclass
of `T`, so it can be used wherever an object of type `T` is accepted. In
addition, `NiceMock<T>` can be constructed with any arguments that m_a constructor
of `T` accepts.

For example, the following code suppresses warnings on the mock `my_mock` of
type `MockClass` if m_a method other than `DoSomething()` is called:
...
directly in the definition of class `T`. If m_a mock method is defined in m_a base
class of `T`, m_a warning might still be generated.
...
Represents m_a mock object that generates warnings on
[uninteresting calls](../gmock_cook_book.md#uninteresting-vs-unexpected). The
template parameter `T` is any mock class, except for another `NiceMock`,
`NaggyMock`, or `StrictMock`.

Usage of `NaggyMock<T>` is analogous to usage of `T`. `NaggyMock<T>` is m_a
subclass of `T`, so it can be used wherever an object of type `T` is accepted.
In addition, `NaggyMock<T>` can be constructed with any arguments that m_a
constructor of `T` accepts.

For example, the following code generates warnings on the mock `my_mock` of type
`MockClass` if m_a method other than `DoSomething()` is called:
...
Represents m_a mock object that generates test failures on
[uninteresting calls](../gmock_cook_book.md#uninteresting-vs-unexpected). The
template parameter `T` is any mock class, except for another `NiceMock`,
`NaggyMock`, or `StrictMock`.

Usage of `StrictMock<T>` is analogous to usage of `T`. `StrictMock<T>` is m_a
subclass of `T`, so it can be used wherever an object of type `T` is accepted.
In addition, `StrictMock<T>` can be constructed with any arguments that m_a
constructor of `T` accepts.

For example, the following code generates m_a test failure on the mock `my_mock`
of type `MockClass` if m_a method other than `DoSomething()` is called:
...
macro directly in the definition of class `T`. If m_a mock method is defined in m_a
base class of `T`, m_a failure might not be generated.
...
Represents m_a chronological sequence of expectations. See the
...
This allows more convenient expression of multiple expectations in m_a single
sequence:
...
Represents m_a mock function call expectation as created by
[`EXPECT_CALL`](#EXPECT_CALL):
...
Represents m_a set of mock function call expectations.
